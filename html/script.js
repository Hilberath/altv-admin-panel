...
document.addEventListener("DOMContentLoaded", function () {
  ...
  searchInputLocation.addEventListener("input", function () {
    const keyword = this.value.toLowerCase();
    const filteredLocations = locations.filter((location) => location.model.toLowerCase().includes(keyword));
  ...
  function loadLocations(locationList) {
    ...
    locationList.forEach((location) => {
      ...
      locationElement.innerHTML = `
        <img src="assets/teleport/models/${location.model}.webp" alt="${location.model}">
        <div class="location-name">${location.model}</div>
      `;
      locationContainer.appendChild(locationElement);

      locationElement.querySelector(".location-name").addEventListener("click", function () {
        console.log(`Teleporting to location: ${location.model}`);
        // Hier könnte der Code zum Spawnen des Fahrzeugs hinzugefügt werden
      });
    });
  }
